...
class TurretGame extends React.Component {
  ...
  constructor(props) {
    ...
    this.fireSpd = 0.25;
    this.spdPerSummon = 500;
    this.minSummonSpd = 500;
    this.zombieSpd = 0.05;
    this.zombieDelay = 500;
    this.lifeCount = 1;
    this.center = { x: 4, y: 4 };
  ...
  summonZombieTrigger() {
    if (this.state.summonTime > this.state.summonSpd) {
      ...
      let nextSummonSpd =
        this.state.summonSpd > this.minSummonSpd
          ? this.state.summonSpd - 100
          : this.minSummonSpd;
    ...
    this.objectCollision(
      ...
      (i, j, tmpArr1, tmpArr2) => {
        ...
        if (zombies[i].c.indexOf("zombie-hiding") < 0) {
          zombies[i].c = zombies[i].c.replace(
            /zombie-walking/g,
            "zombie-hiding"
          );
          zombies[i].s = true;
  ...
  animateMoveByArray(arr, spd, collision, delay = 0, callback = null) {
    ...
    for (let i = 0; i < tmpArr.length; i++) {
      if (
        (delay > 0 && performance.now() - tmpArr[i].d < delay) ||
        tmpArr[i].s === true
      ) {
        continue;
      }
  ...
  renderByArray(className, object, innerJSX = null, callback = null) {
    ...
    for (let i = 0; i < objs.length; i++) {
      ...
      let addClass =
        typeof obj.c !== "undefined" ? `${className} ${obj.c}` : className;

      if (callback === null) {
        objArray.push(
          <div className={addClass} style={style}>
            {innerJSX}
          </div>
        );
      } else {
  ...
  renderZombie() {
    return this.renderByArray(
      ...
      (i, event) => {
        if (
          ["zombie-fade-out", "zombie-hiding"].indexOf(event.animationName) >= 0
        ) {
          let zombies = this.state.zombies;
  ...
  renderLife() {
    ...
    for (let i = 1; i <= this.lifeCount; i++) {
      let lifeClassName = i <= this.state.life ? "heart full" : "heart empty";

      lifeArray.push(<div className={lifeClassName}></div>);
    }

    return <div className="life">{lifeArray}</div>;
  }
  ...
  renderWord(words, size = "") {
    ...
    for (let i in words) {
      ...
      returnJSX.push(<div className="title-content">{tmpJSX}</div>);
    }
  ...
  renderTitle() {
    ...
    let titleText =
      this.state.showTryAgain === false ? ["zombie", "shooter"] : ["you lose"];
    let subTitleText =
    ...
    return (
      <div className={titleClass}>
        <div className="game-title">{titleJSX}</div>
        <div className="game-title">{subTitleJSX}</div>
        <div className="game-title">{startJSX}</div>
      </div>
    );
  }
...
ReactDOM.render(<TurretGame />, document.querySelector("#root"));
